...
// Godtar «7:30», «07:30» og «07:30:00» og gir alltid HH:MM:SS tilbake, som er
// det eneste Sonos aksepterer.
...
function validateAlarm(alarm) {
  if (!normalizeTime(alarm.startTime)) {
    throw new Error(`Ugyldig starttid: ${alarm.startTime}`);
  }
  if (!normalizeTime(alarm.duration)) {
    throw new Error(`Ugyldig varighet: ${alarm.duration}`);
  }
  if (!isValidRecurrence(alarm.recurrence)) {
    throw new Error(`Ugyldig gjentakelse: ${alarm.recurrence}`);
  }
  if (!alarm.roomUUID) {
    throw new Error('Alarmen mangler rom (RoomUUID)');
  }
  if (!alarm.programURI) {
    throw new Error('Alarmen mangler lydkilde (ProgramURI)');
  }
  return true;
}
