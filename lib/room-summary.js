...
// Sammendraget for romkortet. Flisa viser rundt tjue tegn, sa den kan ikke
// baere hele lista — fem alarmer ble kuttet etter den forste uansett hvordan
// de ble skilt. Den viser derfor den tidligste alarmen med gjentakelsen sin,
// og hvor mange flere som finnes. Hele lista star i enhetsinnstillingene.
// `lead` er alarmen som skal sta forst — den som faktisk ringer neste gang.
// Uten den ville den tidligste PA KLOKKA topper lista, og en engangsalarm kl.
// 06:00 ville skygget for hele ukeplanen bak «+4» — selv etter at Sonos har
// slatt den av. Er ingen alarm aktiv, faller vi tilbake pa den tidligste.
function describeRoom(alarms, language = 'en', lead = null) {
  const list = sortedByTime(alarms);
  if (list.length === 0) return language === 'no' ? 'Ingen alarmer' : 'No alarms';

  const first = list.find((alarm) => lead && alarm.id === lead.id) || list[0];
  const time = String(first.startTime || '').slice(0, 5);
  const when = describeRecurrence(first.recurrence, language);
  // Ingen av/pa-markor her. Den ville bare beskrevet den forste alarmen mens
  // «+4» kan vaere hva som helst, og rommets egen bryter sier allerede om noe
  // er pa. To tegn spart, og tvetydigheten borte.
  const more = list.length > 1 ? ` +${list.length - 1}` : '';

  return `${time} ${when}${more}`;
}
